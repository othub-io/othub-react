...
const AssetPublish = (selectedFile) => {
  const { setIsLoading, setData, setResultValue, isLoading } =
  ...
  useEffect(() => {
    async function readFile() {
      try {
        ...
        const rsp = await axios.get('https://api.coingecko.com/api/v3/coins/origintrail');
  ...
  if (isLoading) {
    return (
      <Loading data={"Please sign awaiting transaction(s)..."}/>
    );
  }
        ...
        {result.status ? (
          <div>
            <div className="publish-asset-div">
              <div className="publish-asset-ual"><strong>UAL:</strong><br></br>{result.result.UAL}</div>
              <div className="publish-asset-assertion"><strong>Assertion ID:</strong><br></br>{result.result.publicAssertionId}</div>
            </div>
            
            <div className="publish-asset-link"><a href={`${explorer_url}/explore?ual=${result.result.UAL}`} target="_blank" className="publish-asset-link">Explore</a>:<br></br> 
            <span style={{paddingLeft:"10px",fontSize: "14px"}}>Explore your asset's data on the official DKG Explorer!</span></div>
            <br></br>
            <div className="publish-asset-link"><a href={`https://othub.io/my-othub/inventory?ual=${result.result.UAL}`} target="_blank" className="publish-asset-link">Inventory</a>:<br></br>
            <span style={{paddingLeft:"10px",fontSize: "14px"}}>Open your inventory to view your asset's details!</span></div>
          </div>
        ) : (
  ...
  return (
    !result && <div className="publish-asset">
      <div className="publish-asset-title">
        Are you sure you want to publish {selectedFile.data.name} to {chain_id}?
      </div>
      ...
      <div className="publish-asset-estimated-cost">
        ...
        {formatBytes(selectedFile.data.size)}<br></br> 
        x{inputValue}  Epochs ({inputValue * 90} Days)<br></br>
         x3  Nodes<br></br>
         {cost !== 0 ? (`${cost} TRAC ($${(cost * price).toFixed(3)})`) : ("Estimating cost...") }
        </span>
      </div>
