...
const TotalEarnings = (node_data) => {
  ...
  for(let i = 0; i < Number(data.length);i++){
    totalEarnings = totalEarnings + Number(data[i].cumulativeEstimatedEarnings)
  }
  ...
  return (
    <div>
      <div className="dashboard-total-rewards-title">Total Est. Earnings</div>
      <div className="dashboard-total-rewards-value">{totalEarnings.toFixed(3)}<span>TRAC</span></div>
    </div>
  );
};
