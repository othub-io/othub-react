  ...
  if (nodeData) {
    ...
    for (const blockchain of nodeData) {
      ...
      for (const obj of formattedData.labels) {
        let containsDate = blockchain.data.some((item) => moment(button === "24h" || button === "7d" ? (item.datetime) : (item.date)).format(format) === obj && item.tokenName === tokenName);
        if(containsDate){
          for (const item of blockchain.data) {
            if (moment(button === "24h" || button === "7d" ? (item.datetime) : (item.date)).format(format) === obj && item.tokenName === tokenName) {
